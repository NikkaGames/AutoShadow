...
struct monoDictionary {
    ...
    [[maybe_unused]] bool ContainsKey(TKey key) { }
...
// Helper function for ObscuredLong if not already available
int64_t GetObscuredLongValue(uint64_t location) {
    int64_t cryptoKey = *reinterpret_cast<int64_t*>(location);
    int64_t obfuscatedValue = *reinterpret_cast<int64_t*>(location + 8);
    return obfuscatedValue ^ cryptoKey;
}
